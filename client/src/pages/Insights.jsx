    ...
    <div className="p-4 md:px-10 md:py-6 flex flex-col">
      ...
      {showPriceLoading && (
        <div className="mb-3 shrink-0">
          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-[var(--fill-quaternary)] rounded-full text-[12px] text-[var(--label-tertiary)]">
            <span className="w-1.5 h-1.5 bg-[var(--chart-primary)] rounded-full animate-pulse" />
            Fetching prices...
          </span>
        </div>
      )}
      ...
      <div className="md:hidden overflow-x-auto flex gap-1.5 pb-3 shrink-0 -mx-1 px-1">
        ...
        <motion.button
          ...
          className="shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-full text-[14px] font-medium bg-[var(--fill-tertiary)] text-[var(--label-tertiary)] transition-all"
        >
      ...
      <div className="flex gap-0 flex-1">
        ...
        <nav className="hidden md:flex w-[220px] shrink-0 flex-col pr-4 border-r border-[var(--separator-opaque)] mr-0 py-1 sticky top-0 self-start">
          ...
          <button
            onClick={data.openManageModal}
            className="flex items-center gap-2 px-4 py-2.5 mt-3 pt-3 border-t border-[var(--separator-opaque)] text-[var(--label-tertiary)] hover:text-[var(--label-primary)] transition-colors text-[14px] font-medium"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 010 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 01-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 01-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 01-1.369-.49l-1.297-2.247a1.125 1.125 0 01.26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 010-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z" />
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0Z" />
          ...
          >
            ...
            >
              <div className="px-5 py-4 border-b border-[var(--separator-opaque)]">
                <h2 className="text-[18px] font-semibold text-[var(--label-primary)]">Manage Insight Cards</h2>
                <p className="text-[14px] text-[var(--label-tertiary)] mt-0.5">Choose which cards to display</p>
              </div>
                  ...
                  {data.CARD_CONFIG.map((card) => (
                    <label key={card.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[var(--fill-quaternary)] transition-colors cursor-pointer">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[15px] font-medium text-[var(--label-primary)]">{card.name}</p>
                        <p className="text-[13px] text-[var(--label-tertiary)]">{card.description}</p>
                      </div>
                    </label>
                  ))}
                </div>
              ...
              <div className="px-5 py-4 border-t border-[var(--separator-opaque)] flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={data.cancelPreferences} disabled={data.savingPrefs} className="px-4 py-2 text-[14px] font-medium text-[var(--label-secondary)] hover:bg-[var(--fill-quaternary)] rounded-lg transition-colors disabled:opacity-50">
                    Cancel
                  </button>
                  <button onClick={data.savePreferences} disabled={data.savingPrefs} className="px-4 py-2 text-[14px] font-medium text-white bg-[var(--system-blue)] hover:bg-[var(--system-blue)]/90 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-2">
                    {data.savingPrefs ? (
