...
export default function RiskDiversificationCard({
  ...
  const scoreColor = diversificationScore >= 70 ? '#10B981' : diversificationScore >= 50 ? '#F59E0B' : '#EF4444';
  ...
  if (assetsCount === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-[14px] font-medium text-[var(--label-secondary)] mb-1">No Risk Data</p>
        <p className="text-[12px] text-[var(--label-tertiary)]">Add assets to see risk analysis</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-[13px] text-[var(--label-tertiary)] mt-0.5">Portfolio health and diversification score</p>
      </div>
      ...
      <div className="flex justify-center mb-8">
        <div className="relative w-[180px] h-[180px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-[42px] font-bold text-[var(--label-primary)] tabular-nums leading-none">{diversificationScore}</span>
            <span className="text-[13px] text-[var(--label-tertiary)] mt-1">out of 100</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center mb-6">
        <div className={`inline-flex px-4 py-1.5 rounded-full text-[13px] font-semibold ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Left: Sub-Scores */}
        <div>
          <h4 className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Sub-Scores</h4>
          <div className="space-y-3">
            {subScores.map(item => (
              <div key={item.label} className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[13px] font-medium text-[var(--label-primary)]">{item.label}</span>
                  <span className="text-[14px] font-bold tabular-nums text-[var(--label-primary)]">{item.score}/{item.max}</span>
                </div>
                <div className="h-2.5 bg-[var(--fill-tertiary)] rounded-full overflow-hidden mb-2">
                  <motion.div
                    className="h-full rounded-full"
                    style={{ backgroundColor: item.score / item.max > 0.7 ? '#10B981' : item.score / item.max > 0.4 ? '#F59E0B' : '#EF4444' }}
                    initial={{ width: 0 }}
                    animate={{ width: `${(item.score / item.max) * 100}%` }}
                    transition={{ duration: 0.8, ease: [0.4, 0, 0.2, 1] }}
                  />
                </div>
                <p className="text-[11px] text-[var(--label-tertiary)]">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <h4 className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Portfolio Health</h4>
          <div className="space-y-3">
            {largestHoldingName && (
              <div className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
                <p className="text-[12px] text-[var(--label-tertiary)] mb-1">Largest Holding</p>
                <div className="flex items-center justify-between">
                  <p className="text-[14px] font-semibold text-[var(--label-primary)] truncate mr-3">{largestHoldingName}</p>
                  <p className="text-[20px] font-bold tabular-nums shrink-0" style={{ color: largestHoldingWeight > 30 ? '#F59E0B' : '#10B981' }}>
            ...
            {missingCategories && missingCategories.length > 0 && (
              <div className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
                <p className="text-[12px] text-[var(--label-tertiary)] mb-2">Not invested in</p>
                <div className="flex flex-wrap gap-1.5">
                  {missingCategories.map(cat => (
                    <span key={cat} className="px-2 py-1 text-[11px] font-medium bg-[var(--fill-tertiary)] text-[var(--label-secondary)] rounded-lg">
                      {cat}
                    </span>
                  ))}
                </div>
            ...
            {riskRecommendation && (
              <div className="p-3 bg-[var(--system-blue)]/5 border border-[var(--system-blue)]/15 rounded-xl">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-[12px] text-[var(--label-secondary)] leading-relaxed">{riskRecommendation}</p>
                </div>
              </div>
            )}
          </div>
