  ...
  if (!hasAnyTaxData) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-[12px] text-[var(--label-tertiary)]">Add equity, fixed income, or real estate<br />assets to see tax implications</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        <p className="text-[13px] text-[var(--label-tertiary)] mt-0.5">Estimated capital gains and income tax</p>
      </div>

      {/* Hero: Total Tax */}
      {hasEquityAssets && (
        <div className="mb-6 p-5 bg-gradient-to-br from-[#EF4444]/10 to-[#DC2626]/5 border border-[#EF4444]/20 rounded-2xl text-center">
          <p className="text-[12px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-2">Estimated Equity Tax</p>
          <p className="text-[40px] font-bold text-[#EF4444] tabular-nums leading-none">{formatCurrency(totalTax)}</p>
          <div className="flex items-center justify-center gap-4 mt-4">
            <div className="px-4 py-2 bg-[var(--bg-primary)]/60 rounded-xl">
              <p className="text-[10px] font-semibold text-[#059669] mb-0.5">LTCG @ 12.5%</p>
              <p className="text-[16px] font-bold text-[var(--label-primary)] tabular-nums">{formatCurrency(ltcgTax)}</p>
            </div>
            <div className="px-4 py-2 bg-[var(--bg-primary)]/60 rounded-xl">
              <p className="text-[10px] font-semibold text-[#F59E0B] mb-0.5">STCG @ 20%</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {hasEquityAssets && (
          <div className="p-4 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
            <h4 className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">LTCG Exemption</h4>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[12px] text-[var(--label-tertiary)]">&#8377;1.25L annual exemption</span>
              <span className="text-[13px] font-bold tabular-nums text-[var(--label-primary)]">{ltcgExemptionPercent.toFixed(0)}%</span>
            </div>
            <div className="h-3 bg-[var(--fill-tertiary)] rounded-full overflow-hidden mb-3">
              <motion.div
                className="h-full rounded-full"
                style={{ backgroundColor: ltcgExemptionPercent >= 100 ? '#EF4444' : ltcgExemptionPercent >= 75 ? '#F59E0B' : '#10B981' }}
                initial={{ width: 0 }}
        ...
        {/* Tax-Loss Harvesting */}
        {totalLoss > 0 ? (
          <div className="p-4 bg-[#3B82F6]/5 border border-[#3B82F6]/15 rounded-xl">
            <h4 className="text-[13px] font-semibold text-[#3B82F6] uppercase tracking-wide mb-3">Tax-Loss Harvesting</h4>
            <p className="text-[22px] font-bold text-[var(--label-primary)] tabular-nums mb-1">{formatCompact(totalLoss)}</p>
            <p className="text-[11px] text-[var(--label-tertiary)] mb-2">Unrealized losses can offset gains</p>
            {ltcgLoss > 0 && stcgLoss > 0 && (
              <div className="flex items-center gap-3 text-[11px] text-[var(--label-tertiary)] tabular-nums">
          ...
          <div className="p-4 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60 flex flex-col justify-center">
            <h4 className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-2">Tax-Loss Harvesting</h4>
            <p className="text-[12px] text-[var(--label-tertiary)]">No unrealized losses to harvest</p>
          </div>
        ) : null}
        ...
        {hasEquityAssets && (
          <>
            <div className="p-3.5 bg-gradient-to-br from-[#059669]/8 to-[#10B981]/4 border border-[#059669]/15 rounded-xl">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[11px] font-semibold text-[#059669]">LTCG (1Y+)</span>
              </div>
              <p className="text-[22px] font-bold text-[var(--label-primary)] tabular-nums mb-0.5">{formatCompact(ltcgGain)}</p>
              <p className="text-[11px] text-[var(--label-tertiary)]">{ltcgAssetCount} asset{ltcgAssetCount !== 1 ? 's' : ''} @ 12.5% above &#8377;1.25L</p>
            </div>
            <div className="p-3.5 bg-gradient-to-br from-[#F59E0B]/8 to-[#D97706]/4 border border-[#F59E0B]/15 rounded-xl">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[11px] font-semibold text-[#F59E0B]">STCG (&lt;1Y)</span>
              </div>
              <p className="text-[22px] font-bold text-[var(--label-primary)] tabular-nums mb-0.5">{formatCompact(stcgGain)}</p>
              <p className="text-[11px] text-[var(--label-tertiary)]">{stcgAssetCount} asset{stcgAssetCount !== 1 ? 's' : ''} @ 20%</p>
            </div>
          </>
        )}
        ...
        {assetBreakdown.length > 0 && (
          <div className="p-4 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60 md:col-span-2">
            <h4 className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Per-Asset Breakdown</h4>
            {/* Table header */}
                ...
                return (
                  <div key={idx} className="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-[var(--fill-quaternary)] transition-colors">
                    <div className="flex-1 min-w-0 flex items-center gap-1.5">
                      <span className="text-[12px] font-medium text-[var(--label-primary)] truncate">{item.name}</span>
                    ...
                    <span className={`w-16 text-right text-[11px] tabular-nums shrink-0 ${item.ltcgGain > 0 ? 'text-[#059669] font-semibold' : 'text-[var(--label-quaternary)]'}`}>
                      {item.ltcgGain > 0 ? formatCompact(item.ltcgGain) : '-'}
                    </span>
                    <span className={`w-16 text-right text-[11px] tabular-nums shrink-0 ${item.stcgGain > 0 ? 'text-[#F59E0B] font-semibold' : 'text-[var(--label-quaternary)]'}`}>
                      {item.stcgGain > 0 ? formatCompact(item.stcgGain) : '-'}
                    </span>
                    <span className={`w-16 text-right text-[11px] tabular-nums shrink-0 ${totalLosses > 0 ? 'text-[#EF4444] font-semibold' : 'text-[var(--label-quaternary)]'}`}>
                      {totalLosses > 0 ? `-${formatCompact(totalLosses)}` : '-'}
                    </span>
                    <span className={`w-20 text-right text-[12px] font-bold tabular-nums shrink-0 ${
                      item.netGain > 0 ? 'text-[#059669]' : item.netGain < 0 ? 'text-[#EF4444]' : 'text-[var(--label-tertiary)]'
                    }`}>
        ...
        {(taxableInterest > 0 || taxExemptInterest > 0 || realEstateCG > 0) && (
          <div className="p-4 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
            <h4 className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Other Taxable Income</h4>
            <div className="space-y-2.5">
              {taxableInterest > 0 && (
                <div className="flex items-center justify-between">
                  <span className="text-[12px] text-[var(--label-tertiary)]">FD Interest (slab rate)</span>
                  <span className="text-[13px] font-semibold text-[var(--label-primary)] tabular-nums">{formatCompact(taxableInterest)}</span>
                </div>
              )}
              {realEstateCG > 0 && (
                <div className="flex items-center justify-between">
                  <span className="text-[12px] text-[var(--label-tertiary)]">Real Estate CG</span>
                  <span className="text-[13px] font-semibold text-[var(--label-primary)] tabular-nums">{formatCompact(realEstateCG)}</span>
                </div>
              )}
              {taxExemptInterest > 0 && (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[12px] text-[var(--label-tertiary)]">PPF/EPF Interest</span>
                    <span className="px-1.5 py-0.5 text-[8px] font-bold rounded bg-[#10B981]/15 text-[#10B981]">EXEMPT</span>
                  </div>
                  <span className="text-[13px] font-semibold text-[var(--label-primary)] tabular-nums">{formatCompact(taxExemptInterest)}</span>
                </div>
              )}
            </div>
            {(taxableInterest > 0 || realEstateCG > 0) && (
              <p className="text-[10px] text-[var(--label-quaternary)] mt-3 pt-2 border-t border-[var(--separator-opaque)]">
