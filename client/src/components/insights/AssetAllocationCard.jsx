  ...
  if (activeAllocation.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-[15px] font-medium text-[var(--label-secondary)] mb-1">No Allocation Data</p>
        <p className="text-[13px] text-[var(--label-tertiary)]">Add assets to see your portfolio allocation</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[14px] text-[var(--label-tertiary)] mt-0.5">{activeAllocation.length} {allocationView === 'Category' ? 'categories' : 'types'} across your portfolio</p>
        </div>
      ...
      <div className="flex justify-center mb-6">
        <div className="relative w-[220px] h-[220px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className={`text-[15px] font-semibold tabular-nums ${totalGainPercent >= 0 ? 'text-[var(--system-green)]' : 'text-[var(--system-red)]'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-[14px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide">
              {allocationView === 'Category' ? 'Categories' : 'Types'}
            </h4>
            {allocationView === 'Category' && targetPercent > 0 && (
              <span className="text-[11px] text-[var(--label-quaternary)]">Target: ~{targetPercent.toFixed(0)}% each</span>
            )}
          </div>
              ...
              <div key={cat.key} className="p-2.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/40">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className={`text-[12px] font-semibold tabular-nums w-14 text-right shrink-0 ${
                    cat.returnPercent > 0 ? 'text-[var(--system-green)]' : cat.returnPercent < 0 ? 'text-[var(--system-red)]' : 'text-[var(--label-tertiary)]'
                  }`}>
                  ...
                  <div className="mt-1.5 flex items-center gap-2">
                    <div className="flex-1 h-1 bg-[var(--fill-tertiary)] rounded-full relative overflow-hidden">
                      ...
                      <div
                        ...
                        style={{
                          backgroundColor: cat.deviation > 0 ? 'var(--system-amber)' : 'var(--chart-primary)',
                    ...
                    <span className={`text-[9px] font-semibold tabular-nums shrink-0 ${
                      cat.deviation > 5 ? 'text-[var(--system-amber)]' : cat.deviation < -5 ? 'text-[var(--chart-primary)]' : 'text-[var(--label-quaternary)]'
                    }`}>
          ...
          {allocationView === 'Category' && meaningfulCategories > 1 && (overWeighted > 0 || underWeighted > 0) && (
            <div className="mt-3 p-2.5 bg-[var(--fill-quaternary)] rounded-xl">
              <div className="flex items-center gap-2 text-[12px] text-[var(--label-tertiary)]">
                ...
                <span>
                  {overWeighted > 0 && <><span className="text-[var(--system-amber)] font-semibold">{overWeighted}</span> over-weighted</>}
                  {overWeighted > 0 && underWeighted > 0 && ', '}
                  {underWeighted > 0 && <><span className="text-[var(--chart-primary)] font-semibold">{underWeighted}</span> under-weighted</>}
          ...
          {bestCategory && worstCategory && (
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-[var(--system-green)]/8 border border-[var(--system-green)]/15 rounded-xl">
                ...
                <p className="text-[18px] font-bold text-[var(--system-green)] tabular-nums" style={{ fontFamily: 'var(--font-display)' }}>+{bestCategory.returnPercent.toFixed(1)}%</p>
                <p className="text-[11px] text-[var(--system-green)]/70 tabular-nums">+{formatCompact(bestCategory.returnAmount)}</p>
              </div>
              <div className="p-3 bg-[var(--system-red)]/8 border border-[var(--system-red)]/15 rounded-xl">
                ...
                <p className="text-[18px] font-bold text-[var(--system-red)] tabular-nums" style={{ fontFamily: 'var(--font-display)' }}>{worstCategory.returnPercent.toFixed(1)}%</p>
                <p className="text-[11px] text-[var(--system-red)]/70 tabular-nums">{formatCompact(worstCategory.returnAmount)}</p>
              </div>
            </div>
          )}
          </div>
