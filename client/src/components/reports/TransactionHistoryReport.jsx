...
export default function TransactionHistoryReport({ transactions, summary, loading }) {
  ...
  if (loading) {
    return (
      <div className="py-16 flex flex-col items-center gap-3">
        ...
        <p className="text-[13px] text-[var(--label-tertiary)]">Loading transactions...</p>
      </div>
    );
  }
  ...
  if (!transactions || transactions.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-[14px] font-medium text-[var(--label-secondary)] mb-1">No Transactions</p>
        <p className="text-[12px] text-[var(--label-tertiary)]">Add transactions to see your history</p>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Title */}
      <div className="mb-6">
        <h2 className="text-[18px] font-bold text-[var(--label-primary)]">Transaction History</h2>
        <p className="text-[13px] text-[var(--label-tertiary)] mt-0.5">{transactions.length} transactions across your portfolio</p>
      </div>

      {/* Hero: Stat Blocks 2x2 */}
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
          <p className="text-[11px] text-[var(--label-tertiary)] font-medium uppercase tracking-wide mb-1">Total Transactions</p>
          <p className="text-[22px] font-bold text-[var(--label-primary)] tabular-nums">{summary?.totalTransactions || 0}</p>
        </div>
        <div className="p-3.5 bg-[#059669]/8 border border-[#059669]/15 rounded-xl">
          <p className="text-[11px] text-[#059669] font-medium uppercase tracking-wide mb-1">Total Buys</p>
          <p className="text-[20px] font-bold text-[var(--label-primary)] tabular-nums">{formatCompact(summary?.totalBuys || 0)}</p>
        </div>
        <div className="p-3.5 bg-[#DC2626]/8 border border-[#DC2626]/15 rounded-xl">
          <p className="text-[11px] text-[#DC2626] font-medium uppercase tracking-wide mb-1">Total Sells</p>
          <p className="text-[20px] font-bold text-[var(--label-primary)] tabular-nums">{formatCompact(summary?.totalSells || 0)}</p>
        </div>
        <div className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
          <p className="text-[11px] text-[var(--label-tertiary)] font-medium uppercase tracking-wide mb-1">Net Flow</p>
          <p className="text-[20px] font-bold text-[var(--label-primary)] tabular-nums">{formatCompact(summary?.netFlow || 0)}</p>
        </div>
      </div>

      {/* Recent Transactions */}
      <div>
        <h4 className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Recent Transactions</h4>
        {/* Table header */}
        ...
        <div className="space-y-0.5 mt-1">
          {displayTransactions.map((txn, idx) => (
            <div key={idx} className="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-[var(--fill-quaternary)] transition-colors">
              <span className="w-20 text-[11px] text-[var(--label-tertiary)] tabular-nums shrink-0">
                {new Date(txn.transaction_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
              </span>
              <div className="flex-1 min-w-0 flex items-center gap-1.5">
                <span className="text-[12px] font-medium text-[var(--label-primary)] truncate">{txn.assetName}</span>
              ...
              <span className="w-14 text-center shrink-0">
                <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                  txn.type === 'BUY' ? 'bg-[#059669]/10 text-[#059669]' : 'bg-[#DC2626]/10 text-[#DC2626]'
                }`}>
                  {txn.type}
                </span>
              </span>
