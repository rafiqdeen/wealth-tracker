...
export default function GoalProgressReport({ goals, summary }) {
  ...
  if (!goals || goals.length === 0) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-[15px] font-medium text-[var(--label-secondary)] mb-1">No Goals</p>
        <p className="text-[13px] text-[var(--label-tertiary)]">Create goals to track your financial progress</p>
      </div>
    );
  }
  ...
  const scoreColor = overallPercent >= 70 ? 'var(--system-green)' : overallPercent >= 40 ? 'var(--system-amber)' : 'var(--system-red)';
  ...
  const levelColor = overallPercent >= 70 ? 'var(--system-green)' : overallPercent >= 40 ? 'var(--system-amber)' : 'var(--system-red)';
  ...
  return (
    <div>
      ...
      <div className="flex justify-center mb-4">
        <div className="relative w-[180px] h-[180px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[14px] text-[var(--label-tertiary)] mt-1">percent</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center mb-6">
        <div className={`inline-flex px-4 py-1.5 rounded-full text-[14px] font-semibold`} style={{ backgroundColor: `${levelColor}20`, color: levelColor }}>
          {level}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Left: Goals */}
        <div>
          <h4 className="text-[14px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Goals</h4>
          <div className="space-y-2">
            {displayGoals.map((goal, index) => {
              const progress = goal.progress || 0;
              const progressColor = progress >= 100 ? 'var(--system-green)' : progress >= 50 ? 'var(--system-amber)' : goal.categoryConfig?.color || '#6B7280';
              const statusLabel = progress >= 100 ? 'Completed' : progress >= 75 ? 'Almost' : progress >= 25 ? 'In Progress' : 'Started';
              const statusBg = progress >= 100 ? 'bg-[var(--system-green)]/15 text-[var(--system-green)]' : progress >= 50 ? 'bg-[var(--system-amber)]/15 text-[var(--system-amber)]' : 'bg-[var(--fill-tertiary)] text-[var(--label-tertiary)]';
              return (
                <div key={goal.id || index} className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[14px] font-medium text-[var(--label-primary)] truncate mr-2">{goal.name}</p>
        ...
        {/* Right: Summary */}
        <div>
          <h4 className="text-[14px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Summary</h4>
          <div className="space-y-3">
            {/* Completed / Total */}
            <div className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
              <p className="text-[13px] text-[var(--label-tertiary)] mb-1">Completed</p>
              <div className="flex items-baseline gap-1">
                <span className="text-[28px] font-bold text-[var(--system-green)] tabular-nums" style={{ fontFamily: 'var(--font-display)' }}>{completed}</span>
                <span className="text-[15px] text-[var(--label-tertiary)] tabular-nums">/ {totalGoals}</span>
              </div>
            </div>
            ...
            <div className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[13px] text-[var(--label-tertiary)]">Total Saved vs Target</p>
                <p className="text-[14px] font-bold tabular-nums text-[var(--label-primary)]">{overallPercent.toFixed(0)}%</p>
              </div>
            ...
            {behindSchedule > 0 && (
              <div className="p-3 bg-[var(--system-orange)]/8 border border-[var(--system-orange)]/15 rounded-xl">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-[12px] text-[var(--system-orange)] leading-relaxed">
