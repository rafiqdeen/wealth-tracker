  ...
  if (!hasAnyData) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="text-[15px] font-medium text-[var(--label-secondary)] mb-1">No Tax Data</p>
        <p className="text-[13px] text-[var(--label-tertiary)]">Add income-generating assets to see tax summary</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 p-5 bg-gradient-to-br from-[var(--system-red)]/10 to-[var(--system-red)]/5 border border-[var(--system-red)]/20 rounded-2xl text-center">
        <p className="text-[13px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-2">Total Taxable Income</p>
        <p className="text-[40px] font-bold text-[var(--system-red)] tabular-nums leading-none">{formatCurrency(totalTaxableIncome || 0)}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <h4 className="text-[14px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Income Breakdown</h4>
          <div className="space-y-3">
            {/* Taxable Interest */}
            <div className="p-3.5 bg-[var(--bg-secondary)] rounded-xl border border-[var(--separator-opaque)]/60">
              <p className="text-[13px] text-[var(--label-tertiary)] mb-1">Taxable Interest</p>
              <p className="text-[22px] font-bold text-[var(--label-primary)] tabular-nums" style={{ fontFamily: 'var(--font-display)' }}>{formatCompact(taxableInterestIncome || 0)}</p>
              <p className="text-[11px] text-[var(--label-quaternary)] mt-1">FD, RD interest — taxed at slab rate</p>
            </div>

            {/* Capital Gains */}
            <div className="p-3.5 bg-gradient-to-br from-[var(--system-amber)]/8 to-[#D97706]/4 border border-[var(--system-amber)]/15 rounded-xl">
              <p className="text-[13px] text-[var(--label-tertiary)] mb-1">Capital Gains (Unrealized)</p>
              <p className="text-[22px] font-bold text-[var(--label-primary)] tabular-nums" style={{ fontFamily: 'var(--font-display)' }}>{formatCompact(totalCapitalGains || 0)}</p>
              <p className="text-[11px] text-[var(--label-quaternary)] mt-1">Equity gains — tax on sell only</p>
            </div>
            ...
            {(taxExemptInterestIncome || 0) > 0 && (
              <div className="p-3.5 bg-[var(--system-green)]/8 border border-[var(--system-green)]/15 rounded-xl">
                <div className="flex items-center gap-2 mb-1">
                  <p className="text-[13px] text-[var(--label-tertiary)]">Tax-Exempt Interest</p>
                  <span className="px-1.5 py-0.5 text-[8px] font-bold rounded bg-[var(--system-green)]/15 text-[var(--system-green)]">EXEMPT</span>
                </div>
                <p className="text-[22px] font-bold text-[var(--system-green)] tabular-nums" style={{ fontFamily: 'var(--font-display)' }}>{formatCompact(taxExemptInterestIncome)}</p>
                <p className="text-[11px] text-[var(--system-green)]/70 mt-1">PPF, EPF, VPF, SSY (EEE instruments)</p>
              </div>
            )}
          </div>
        </div>
        ...
        {assetList.length > 0 && (
          <div>
            <h4 className="text-[14px] font-semibold text-[var(--label-secondary)] uppercase tracking-wide mb-3">Interest Details</h4>
            {/* Table header */}
            ...
            <div className="space-y-0.5 mt-1">
              {displayAssets.map((asset, idx) => (
                <div key={idx} className="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-[var(--fill-quaternary)] transition-colors">
                  ...
                  <span className="w-16 text-right text-[12px] text-[var(--label-tertiary)] tabular-nums shrink-0">
                    {formatCompact(asset.principal)}
                  </span>
                  <span className="w-16 text-right text-[12px] font-semibold text-[var(--system-green)] tabular-nums shrink-0">
