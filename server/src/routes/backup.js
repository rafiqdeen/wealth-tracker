...
router.get('/export', (req, res) => {
  try {
    ...
    const goalAssetLinks = db.prepare(`
    ...
    const goalContributions = db.prepare(`
    ...
    const portfolioHistory = db.prepare(`
    ...
    if (importAssets && data.assets && Array.isArray(data.assets)) {
      ...
      for (const asset of data.assets) {
        try {
          ...
          `).get(userId, asset.name, asset.category);
      ...
      if (importTransactions && data.transactions && Array.isArray(data.transactions)) {
        for (const txn of data.transactions) {
          try {
            ...
            `).get(newAssetId, userId, txn.type, txn.quantity, txn.transaction_date);
